...
def test_all_case(net, param, testloader, stride_xy=64, stride_z=64, draw_ddm_im=False):
    
    logging.info(param)

    total_metric_c = np.zeros((param.n_coarse - 1, n_eval))
    total_metric_f = np.zeros((param.n_fine, n_eval))
    all_total_metric_f = np.zeros((len(testloader), param.n_fine - 1, n_eval))
    n_images = len(testloader)

    tsne_index = 0 if draw_ddm_im else -1
    print("Testing begin")
    with open(os.path.join(param.path_to_dataset, 'test.list'), 'r') as fp:
    ...
    for case_index, sampled_batch in enumerate(tqdm(testloader)):
        ...
        metric_c, metric_f, feat_map = test_single_case(
            net, param, sampled_batch, stride_xy, stride_z, save_pred=False, ids=ids
        )
        
        if case_index == tsne_index:
            if not visualize(
                feat_map, sampled_batch['fine'][0], 0, 'tsne', param,
                os.path.join(param.path_to_test, f'tsne_{param.exp_name}.eps'),
                legend=param.legend, n_components=2
            ):
                tsne_index += 1
        
        for c in range(param.n_coarse - 1):
            total_metric_c[c] += metric_c[c]
            logging.debug(f'{ids}\t' + '\t'.join([f"{metric_c[c, k]:.3f}" for k in range(n_eval)]))
            
        for f in range(param.n_fine - 1):
    ...
    mean_f = [total_metric_f[:, i].sum() / (param.n_fine - 1) for i in range(len(eval_metrics))]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default='/nas/dailinrui/SSL4MIS/model/ACDC', help='root path for training model')
    parser.add_argument('--exp_name', type=str, default='unet24', help='name of the current model')
    args = parser.parse_args()
    
    with open(os.path.join(args.model_path, args.exp_name, 'param.json'), 'r') as fp:
        d = json.load(fp)
    ...
    test_all_case(net, param, testloader, stride_xy=64, stride_z=64)
